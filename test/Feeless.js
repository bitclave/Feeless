...
contract('Feeless', function ([_, wallet1, wallet2, wallet3, wallet4, wallet5, wallet6]) {
    ...
    it('should works with feeless', async function() {
        {
            ...
            const data = '0x' + abi.simpleEncode("setValue(uint256)", 400).toString('hex');
            const hash = web3.sha3(data + padLeft(nonce.toString(16), 64), { encoding: 'hex' });
            const sig = await web3.eth.sign(wallet1, hash);

            await feeless.performFeelessTransaction(wallet1, data, nonce, sig);
...

        {
            ...
            const data = '0x' + abi.simpleEncode("setValue(uint256)", 500).toString('hex');
            const hash = web3.sha3(data + padLeft(nonce.toString(16), 64), { encoding: 'hex' });
            const sig = await web3.eth.sign(wallet2, hash);

            await feeless.performFeelessTransaction(wallet2, data, nonce, sig);
    ...
    it('should failure with wrong data', async function() {
        const nonce = await feeless.nonces.call(wallet1);
        //const data = await feeless.contract.setValue(400, { from: _ }).encodeABI();
        const data = '0x' + abi.simpleEncode("setValue(uint256)", 1000).toString('hex');
        const hash = web3.sha3(data + padLeft(nonce.toString(16), 64), { encoding: 'hex' });
        const sig = await web3.eth.sign(wallet1, hash);

        const wrongData = '0x' + abi.simpleEncode("setValue(uint256)", 1001).toString('hex');
        await feeless.performFeelessTransaction(wallet1, wrongData, nonce, sig).should.be.rejectedWith(EVMRevert);
    });

    it('should failure with wrong nonce', async function() {
        const nonce = await feeless.nonces.call(wallet1);
        //const data = await feeless.contract.setValue(400, { from: _ }).encodeABI();
        const data = '0x' + abi.simpleEncode("setValue(uint256)", 2000).toString('hex');
        const hash = web3.sha3(data + padLeft(nonce.toString(16), 64), { encoding: 'hex' });
        const sig = await web3.eth.sign(wallet1, hash);

        const wrongNonce = nonce + 1;
        await feeless.performFeelessTransaction(wallet1, data, wrongNonce, sig).should.be.rejectedWith(EVMRevert);
    });

    it('should failure with wrong signature', async function() {
        const nonce = await feeless.nonces.call(wallet1);
        //const data = await feeless.contract.setValue(400, { from: _ }).encodeABI();
        const data = '0x' + abi.simpleEncode("setValue(uint256)", 3000).toString('hex');
        const hash = web3.sha3(data + padLeft(nonce.toString(16), 64), { encoding: 'hex' });
        const sig = await web3.eth.sign(wallet1, hash);

        const wrongSig = '0xdeadbeef' + sig.substr(10);
        await feeless.performFeelessTransaction(wallet1, data, nonce, wrongSig).should.be.rejectedWith(EVMRevert);
    });
